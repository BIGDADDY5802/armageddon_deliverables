...
def list_notes():
    try:
        ...
        # public   = CloudFront, browsers, and proxies may cache this response
        # max-age  = treat as fresh for 30 seconds, then re-fetch from origin
        # This is what CloudFront reads to determine TTL — the cache policy in
        # Terraform sets default_ttl=0 so CloudFront relies entirely on this header.
        # Cache-Control is preferred over Expires because it uses a relative
        # duration (30s from now) rather than an absolute timestamp, making it
        # immune to clock skew between client, server, and CDN.
        from flask import make_response
        response = make_response(
            "<h3>Notes:</h3>" + "".join([f"<li>{r[1]}</li>" for r in rows]) + "<br><a href='/'>Back</a>"
        )
