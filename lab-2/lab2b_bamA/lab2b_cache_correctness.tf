...
##############################################################
# Lab 2B — Safe Caching for a Public GET Endpoint
#
# Purpose: Demonstrate origin-driven Cache-Control caching.
# CloudFront respects the Cache-Control header the Flask app
# sends (public, max-age=30) rather than using a hardcoded TTL.
#
# This satisfies the directive:
#   - Safe caching on a public GET endpoint
#   - Cache-Control from origin drives TTL (not Terraform)
#   - Evidence via Age header and X-Cache
#   - Explains why Cache-Control > Expires
##############################################################

##############################################################
# Cache Policy — Public GET (origin-driven TTL)
#
# HOW THIS WORKS:
# CloudFront cache TTL is determined by three values in order:
#   1. If origin sends Cache-Control max-age → use that
#   2. If not, fall back to default_ttl
#   3. Never exceed max_ttl
#
# By setting default_ttl = 0, we force CloudFront to rely
# entirely on what the origin sends. If Flask says
# "Cache-Control: public, max-age=30", CloudFront caches
# for 30 seconds. If Flask sends nothing, TTL=0 (no cache).
# This is origin-driven caching — the app controls the TTL,
# not the infrastructure layer.
#
# WHY Cache-Control OVER Expires:
# Expires: Thu, 01 Jan 2026 00:00:00 GMT
#   → Uses an absolute timestamp. If the client clock is
#     wrong (even by minutes), caching breaks silently.
#     Common on IoT, mobile, and misconfigured servers.
#
# Cache-Control: public, max-age=30
#   → Uses a relative duration from time of response.
#     Clock skew is irrelevant. Always correct.
#     RFC 7234 says if both are present, Cache-Control wins.
#     AWS, browsers, and proxies all prefer Cache-Control.
##############################################################

resource "aws_cloudfront_cache_policy" "dawgs-armageddon_cache_public_get01" {
  name    = "${var.project_name}-cache-public-get01"
  comment = "Origin-driven TTL for safe public GET endpoints. Flask sets Cache-Control: public, max-age=30."

  # min_ttl=0: allow CloudFront to cache for 0 seconds if origin says so
  # default_ttl=0: if origin sends NO Cache-Control, do not cache (safe default)
  # max_ttl=60: cap at 60s — prevents origin from accidentally setting huge TTLs
  #             Flask sends max-age=30, which is within this cap and will be honored
  min_ttl     = 0
  default_ttl = 0
  max_ttl     = 60

  parameters_in_cache_key_and_forwarded_to_origin {
    # No cookies in cache key — this is a PUBLIC endpoint.
    # If cookies were included, each logged-in user would generate
    # a unique cache entry, fragmenting the cache into uselessness.
    cookies_config { cookie_behavior = "none" }

    # No headers in cache key — same reason as cookies.
    # Accept-Language, User-Agent etc. would explode cache variations.
    headers_config { header_behavior = "none" }

    # No query strings — /list is a stable public resource.
    # If the endpoint ever needs query params (e.g. /list?page=2),
    # change this to "all" and CloudFront will cache each variation separately.
    query_strings_config { query_string_behavior = "none" }

    # Compress cached responses — reduces bytes transferred from edge to client.
    # CloudFront stores one compressed copy and serves it to clients
    # that send Accept-Encoding: gzip or br.
    enable_accept_encoding_gzip   = true
    enable_accept_encoding_brotli = true
  }
}
