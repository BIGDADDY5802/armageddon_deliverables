...
def add_note():
    ...
    try:
        ...
        cur.close()
        conn.close()
        return f"Added: {note_text} | <a href='/api/list'>View List</a>"
    except Exception as e:
        record_failure(str(e))
...
def public_feed():
    ...
    # ignoring max-age=0. Browsers read max-age=0 and revalidate every time —
    # so the browser always asks CloudFront, but CloudFront serves from cache
    # for 30 seconds before asking the origin again.
    # This is the correct pattern for CDN-only caching with browser pass-through.
    import datetime
