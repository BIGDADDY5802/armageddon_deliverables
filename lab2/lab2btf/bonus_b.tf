...
resource "aws_vpc_security_group_ingress_rule" "dawgs-armageddon_alb_sg01_allow_http" {
  ...
  # Lab 2A: locked from 0.0.0.0/0 to CloudFront managed prefix list only.
  # pl-3b927c52 = AWS-maintained list of all CloudFront edge node CIDRs (us-east-1).
  # Terraform will destroy the old CIDR rule and create this prefix-list rule in its place.
...
resource "aws_security_group" "dawgs-armageddon_alb_sg02" {
  name        = "${var.project_name}-alb-sg02"
  description = "ALB security group - HTTPS"
  vpc_id      = aws_vpc.dawgs-armageddon_vpc01.id
...
resource "aws_vpc_security_group_ingress_rule" "dawgs-armageddon_alb_sg02_allow_https" {
  security_group_id = aws_security_group.dawgs-armageddon_alb_sg02.id
  cidr_ipv4         = "0.0.0.0/0"
  from_port         = 443
  ip_protocol       = "tcp"
  to_port           = 443
...
resource "aws_vpc_security_group_egress_rule" "dawgs-armageddon_alb_sg02_allow_all_traffic_ipv4" {
  security_group_id = aws_security_group.dawgs-armageddon_alb_sg02.id
  cidr_ipv4         = "0.0.0.0/0"
  ip_protocol       = "-1"
}
...
resource "aws_lb" "dawgs-armageddon_alb01" {
  ...
  security_groups = [
  aws_security_group.dawgs-armageddon_alb_sg01.id,
  aws_security_group.dawgs-armageddon_alb_sg02.id
]
...
# Lab 2A: default_action changed from forward→TG to fixed-response 403.
# The listener RULE in cloudfront_lab_2a_distribution.tf (priority 1) checks for the
# X-Origin-Verify secret header and forwards only CloudFront-verified requests.
# Everything arriving without the header hits this 403 default. Defense-in-depth.
resource "aws_lb_listener" "dawgs-armageddon_https_listener01" {
  ...
  # Default = BLOCK. The listener rule in cloudfront_lab_2a_distribution.tf
  # fires first (priority 1) and forwards traffic that carries the correct header.
  # Anything without the header lands here and gets 403.
  default_action {
    type = "fixed-response"
    fixed_response {
      content_type = "text/plain"
      message_body = "403 Forbidden - Direct ALB access is not permitted."
      status_code  = "403"
    }
  }
...
# Lab 2A: Record ownership transferred to cloudfront_lab_2a_distribution.tf
# dawgs-armageddon_cf_app_alias now points app.thedawgs2025.click at CloudFront.
# This resource is commented out to prevent Route53 duplicate record conflict.
# resource "aws_route53_record" "dawgs-armageddon_app_alias" {
#   zone_id = var.route53_hosted_zone_id
#   name    = local.dawgs-armageddon_fqdn
#   type    = "A"
#   alias {
