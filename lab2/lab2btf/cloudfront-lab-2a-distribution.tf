...
resource "aws_cloudfront_distribution" "dawgs-armageddon_cf01" {
  ...
  # FIX: S3 origin must be a sibling of the ALB origin block, not nested
  # inside custom_origin_config. Each origin {} is a separate top-level
  # block inside the distribution resource. OAC handles request signing
  # to S3 — no custom_origin_config block is needed or valid here.
  origin {
...
ordered_cache_behavior {
  path_pattern           = "/static/*"
  # FIX: Must point to the S3 origin, not the ALB.
  # Static files live in S3 — sending cache misses to the ALB means your
  # app server handles every static file request, which is slow and costly.
