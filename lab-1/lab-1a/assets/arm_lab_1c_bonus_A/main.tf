...
resource "aws_vpc_security_group_ingress_rule" "chewbacca_ec2_ingress_http" {
...
resource "aws_vpc_security_group_ingress_rule" "chewbacca_ec2_ingress_ssh" {
  security_group_id = aws_security_group.chewbacca_ec2_sg01.id

  cidr_ipv4   = "138.229.171.222/32"
  from_port   = 22
  ip_protocol = "tcp"
  to_port     = 22
}
...
resource "aws_vpc_security_group_egress_rule" "chewbacca_ec2_egress" {
  security_group_id = aws_security_group.chewbacca_ec2_sg01.id
...
resource "aws_vpc_security_group_ingress_rule" "chewbacca_rds_ingress_mysql" {
...
resource "aws_vpc_security_group_egress_rule" "chewbacca_rds_egress" {
  security_group_id = aws_security_group.chewbacca_rds_sg01.id
...
resource "aws_iam_role_policy" "inline_scope" {
  name = "my-custom-inline-policy"
...
# AmazonSSMManagedInstanceCore: allows SSM Session Manager, patch manager, run command
# AWS-managed, regularly updated by AWS — safe to leave as managed attachment
resource "aws_iam_role_policy_attachment" "chewbacca_ec2_ssm_attach" {
  role       = aws_iam_role.chewbacca_ec2_role01.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
}

# CloudWatchAgentServerPolicy: allows the CW agent to push metrics and logs
# AWS-managed, regularly updated by AWS — safe to leave as managed attachment
...
# Instance profile straps the role onto the EC2 at launch
...
#   key_name               = "armageddon-lab-1" # Add this line
